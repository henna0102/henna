...
router.get("/", (req, res) => {
  connection.query(
    "SELECT * FROM ( trips INNER JOIN tripmembers ON trips.tripId = tripmembers.tripId INNER JOIN users ON tripmembers.userId = users.userId) INNER JOIN spots ON trips.spotId = spots.spotId WHERE positionState = 2",
    (error, results1) => {
